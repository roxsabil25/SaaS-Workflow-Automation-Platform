...
const DocumentPendingReview = ({ documents, users, handlePreviewDocument, onRefresh, user }) => {
  ...
  useEffect(() => {
    ...
    switch (filterType) {
      ...
      case 'review':
        filtered = normalized.filter((template) => {
          const status = template.status?.toLowerCase();
          return ['pending_review', 'reviewed'].includes(status) && isUserApprover(template);
        });
        break;
      case 'approval':
        filtered = normalized.filter((template) => {
          const status = template.status?.toLowerCase();
          return ['pending_approval', 'approved'].includes(status) && isUserApprover(template);
        });
        break;
      default:
        filtered = normalized.filter((template) => {
  ...
  return (
    <Card bordered={false}>
      ...
      >
        <Col xs={24} md={12}>
          <Title level={4}>My Documents to Review</Title>
          <Paragraph type="secondary">
            Review documents assigned to you for approval
          </Paragraph>
        </Col>
          ...
          <Space align="start">
            ...
            <div>
              ...
              <ul style={{ paddingLeft: 20, marginTop: 8, marginBottom: 0 }}>
                <li><Text>Only documents where you are assigned as a reviewer/approver are shown</Text></li>
                <li><Text>Documents progress through Review → Approval phases</Text></li>
                <li><Text>Use filters to focus on Review or Approval phase documents</Text></li>
...
export default DocumentPendingReview;